...
void Bullet_Setup(Bullet_PP &pp, size_t &RANGE_LEN, size_t &AGG_NUM)
{
    ...
    // can't randomly choose u, 
    // ECP_random(pp.u);
    EC_POINT_add(group, pp.u, pp.g, pp.h, bn_ctx);
...
void Bullet_Setup(Bullet_PP &pp, size_t &RANGE_LEN, size_t &AGG_NUM, uint8_t *hx, uint8_t hy)
{
    ...
    // C = g^r h^v and v in [0, 2^n-1], so h is G1，g is G2
    BIGNUM *X = BN_new();
    BN_bin2bn(hx, 32, X);
    EC_POINT_set_compressed_coordinates(group, pp.h, X, hy, bn_ctx);
    EC_POINT_copy(pp.g, generator);
    BN_free(X);
    // can't randomly choose u, 
    // ECP_random(pp.u);
